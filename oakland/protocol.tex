...
shared key $k$ with the correct exit proxy's public key $PK_{proxy}$.  \annie{Discuss self-certifying file names here --- 
the format is IP:hostID where hostID is a hash of the proxy's public key~\cite{mazieres2000self}.  The correct exit proxy uses 
this as a proof that he is in the correct part of the consistent hashing circle, and that he is not in the CDN's subnet.} To distribute this value, the publisher 
...
\begin{figure}[t!]
\centering
\includegraphics[width=.5\textwidth]{retrieving_content_new2}
...
\begin{figure}[t!]
\centering
\includegraphics[width=.5\textwidth]{retrieving_content_new_complex2}
\caption{Steps for retrieving content in \system{}.}
...
\begin{figure}[t]
\centering
\includegraphics[width=.45\textwidth]{key_lookup_new}
