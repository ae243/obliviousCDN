...
of \system{} for latency and overhead, as well as scalability.  %In addition to %understanding the latency and overhead produced by the 
%system, we also discuss the scalability of the design and show how \system{} scales with %an increasing number of clients.
...
\system{}; this is shown in Figure \ref{fig:overhead2}. %shows
%the overhead of different components of the system for three different file sizes.  
...
%For evaluating performance, we are also concerned with how \system{} will scale with more users 
%and more URLs.  In particular, w
...
for its set of shared keys/subsequent URLs.  And as the number of URLs increase, the additional load per proxy is low because of the load balancing properties of consistent hashing.  %We also discuss in Section \ref{sec:discussion} how 
