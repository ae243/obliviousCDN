...
{\bf Proxy in Client's Jurisdiction.} By replacing the use of an anonymizing proxy or VPN with the use 
...
{\bf Decouple Content Distribution from Decision of Trust.} Using proxies also separates the issues of 
...
{\bf Increasing the Number of Keys.} If a single shared key $k$ associated with a single content publisher gets 
...
{\bf Adding Secrecy to URL Obfuscation.} According to the strawman approach, the URL is obfuscated via hashing.  Unfortunately, 
