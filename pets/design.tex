...
is a hash of the exit proxy's public key.  \annie{A malicious exit proxy cannot \it{choose} where on the consistent 
hashing ring it sits because it cannot frequently change and re-hash its own IP address (whereas it could re-generate 
a new public key).} When an exit proxy is requesting the shared key $k$ from an origin server, 
...
or {\it F}.  \annie{Recall from Section \ref{sec:attacker} that our threat model does not 
