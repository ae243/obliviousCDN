...
defends against this by using the exit proxies as \annie{{\it natural}} mixes; each exit proxy is receiving 
...
These exit proxies naturally mix the requests enough that the CDN cannot conduct traffic analysis
 and determine which request corresponds to which content on the CDN's cache nodes.\annie{\footnote{\annie{For clarification, the exit proxies 
...
analysis attacks~\cite{wright2009traffic,rackoff1993cryptographic}; \system{} could implement one of these solutions \annie{(such as slight timing delays)} at the exit 
proxy.
