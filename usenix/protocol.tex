...
range of content sizes (i.e., if content is between length x and y, then pad it to length 
...
it cannot see what content it is caching. Once the content is obfuscated, the origin server must also obfuscate the content's identifier.  To do so, 
she computes the HMAC of the URL using the shared key $k$.

Once the identifier and the content are obfuscated with $k$, they can be pushed to the CDN. Recently, services have cropped up to allow and help facilitate the use of multiple CDNs for the same content; an 
...
As the exit proxies use consistent hashing to divide keys among proxies, the origin server
...
then she generates a source route, which includes some of its peers, and could potentially
...
\begin{figure}[t!]
\centering
\includegraphics[width=.4\textwidth]{request_diagram_complex}
\caption{Steps for retrieving content in \system{} when a client is prioritizing 
