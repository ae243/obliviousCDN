...
and remove infringing content~\cite{eu-copyright}\paul{ do these examples still hold? Updates?}. In the United States,
...
that clients request; and (3)~the identity of clients that request it. We call this system an {\em oblivious CDN}~(\system{}).%, because the CDN is oblivious to both the content it is storing and the content that clients request.
...
a client, a proxy\paul{drop this?}, or a publisher, and mount coordinated attacks that depend on
