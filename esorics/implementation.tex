...
plaintext web content, we used Surge~\cite{barford1998generating} to generate a set of files that are representative of real-world file distributions.  
...
for the 
hash function.  %Both the plaintext files and encrypted files are stored on this web server, and for the purposes of evaluation, act as a CDN in \system{}.

\textbf{Exit Proxy.} The exit proxy queries the CDN for
encrypted
content on behalf of a client.  We have implemented a proxy in Go; this proxy
runs on
...
\textbf{Client Proxy.} The client runs a local proxy, which uses the same implementation as the exit proxy, but provides 
...
exit proxy.  When the client receives a response from the exit proxy, it decrypts the content with the session key.   
